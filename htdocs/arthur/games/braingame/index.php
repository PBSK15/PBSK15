...
"Who said, \"You guys stink ... and you're not too smart, either\"?" => "D.W."
	...
	function filename($characterName) {
		return strtolower(preg_replace("/\.| |\_|/", "", $characterName)) . ".gif";
	}
	
	function randomItem($array, $dontSelect) {
		$randomItem = array_rand($array, 1);
		
		if ($dontSelect) {
			for ($i = 0, $o = count($dontSelect); $i < $o; $i++) {
				if ($randomItem == $dontSelect[$i]) return randomItem($array, $dontSelect);
			}
		}
		$index = array_search($randomItem, array_keys($array));
		$randomKey = array_keys($array)[$index];
		$randomValue = $array[$randomKey];
		return array($randomItem, $randomKey, $randomValue, $index);
	}
		
	//some hints are missing due to them being lost to time
	$characterData = [
		'Muffy' => "She's the richest kid in school.",
		'Arthur' => "He's the star of the show.",
		'Baby Kate' => 'Hint not available.',
		'Binky' => 'He is a bit of a bully.',
		'Bionic Bunny' => 'Hint not available.',
		'The Brain' => 'Some people might call this kid a genius.',
		'Buster' => 'This boy loves to eat.',
		'D.W.' => 'Her first name is Dora.',
		'Fern' => 'This girl is pretty quiet.',
		'Francine' => 'This monkey is an excellent athlete.',
		'George' => 'This kid is a ventriloquist.',
		'Grandma Thora' => 'This grandparent loves Bingo.',
		'Grandpa Dave' => 'Hint not available.',
		'Jack' => 'Hint not available.',
		'Mary Moo Cow' => "She wears a costume on her own TV show.",
		'Mr. Haney' => 'He runs the school.',
		'Mr. Ratburn' => 'This teacher is no mouse.',
		'Mr. Read' => 'He cooks!',
		'Mr. Rogers' => 'Hint not available.',
		'Mrs. MacGrady' => 'She works in the lunchroom.',
		'Mrs. Read' => 'Hint not available.',
		'Miss Tingley' => 'Hint not available.',
		'Pal' => 'This character has four legs.',
		'Prunella' => 'She has frizzy hair.',
		'Rubella' =>  'Hint not available.',
		'Sue Ellen' => "She has short curly hair.",
		'The Tibbles' => 'Hint not available.'
	];
		
	$questionData = randomItem($questionsAndAnswersArr, null);
	//$questionData = array(array_rand($questionsAndAnswersArr, 1), $questionIndex, $questionsAndAnswersArr[$questionIndex]);
	$answer = $questionData[2];
	$hint = $characterData[$questionData[2]];
	
	switch(rand(0,2)) {
		case 0:
		$imageZero = filename($answer);
		$altZero = $answer;
		
		$dataOne = randomItem($characterData, array($answer));
		$imageOne = filename($dataOne[0]);
		$altOne = $dataOne[0];
		
		$dataTwo = randomItem($characterData, array($answer, $altOne));
		$imageTwo = filename($dataTwo[0]);
		$altTwo = $dataTwo[0];
		
		break;
		
		case 1:
		$imageOne = filename($answer);
		$altOne = $answer;
		
		$dataZero = randomItem($characterData, array($answer));
		$imageZero = filename($dataZero[0]);
		$altZero = $dataZero[0];
		...
		$dataZero = randomItem($characterData, array($answer, $altOne));
		$imageZero = filename($dataZero[0]);
		$altZero = $dataZero[0];
...
<!-- end bbinclude -->
		...
		<div class="ruler"></div>
			<div id="mainFeature" class="braingame">
				<div id="fo_header">
					<div id="brainHint" class="hidden">
						<p>Your hint is:<br><?php echo($hint); ?></p>
					</div>					
					...
					<script type="text/javascript">
						...
						<img src="/arthur/i/games/braingame/braingame100_header.gif" alt="The Brain's Brain Game" class="float-left" width="405" height="102" border="0"><img src="/arthur/i/games/braingame/brainwhelp.gif" alt="Help" usemap="#help" width="195" height="102" border="0">
						<map name="help">
							<area href="/arthur/games/braingame/help.html" shape="rect" coords="105, 20, 190, 70" alt="Help">
						</map>
						<img src="/arthur/i/games/braingame/brain.gif" alt="The Brain holding a piece of papaer" class="brain-right-image" width="196" height="275" border="0">
						<div id="brainGameContent">
							<form method="POST" action="/cgi-registry/arthur/brainquiz2.php" enctype="application/x-www-form-urlencoded" id="brainGameForm">	
								<input type="hidden" name="featurepath" value="/arthur/games/braingame/">
								<input type="hidden" name="answer" value="0">
								<input type="hidden" name="done" value="0">
								<input type="hidden" name="questionindex" value="<?php echo($questionData[3]); ?>">

								<p><?php echo($questionData[1]); ?></p>										
								<p><a href="#" onclick="changeClass('brainHint', 'show')">Hint</a></p>
								
								<?php echo("
								<input type=\"image\" name=\"$altZero\" src=\"/arthur/i/games/braingame/$imageZero\" alt=\"$altZero\" class=\"answer\" width=\"125\" height=\"169\" border=\"0\">
								<input type=\"image\" name=\"$altOne\" src=\"/arthur/i/games/braingame/$imageOne\" alt=\"$altOne\" class=\"answer2\" width=\"125\" height=\"169\" border=\"0\">
								<input type=\"image\" name=\"$altTwo\" src=\"/arthur/i/games/braingame/$imageTwo\" alt=\"$altTwo\" class=\"answer\" width=\"125\" height=\"169\" border=\"0\">\"
								"); ?>
