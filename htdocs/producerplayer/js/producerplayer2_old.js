...
var allow = false;
var whitelist = [
  /^http:\/\/([^\.]+\.)?pbskids\.org/,//pbskids.org subdomains
  /^http:\/\/(www)?(-tc)?\.pbskids\.wested\.org/
];
if( window.top != window ){
  href = document.referrer;

  for(var i in whitelist){
    if(href.match(whitelist[i]) !== null){
      allow = true;
      if(href.match(/\.org\/producerplayer/) !== null)
  ...
  if(!allow){
    throw new Error("This site does not have permission to frame this PBS KIDS video player");
    //return and stop building packages
  }
}
else {
  allow = true;
...
if( allow === true ){
  var cdn  = href.match(/^http:\/\/((?!www(\-tc)?\.).+)pbskids\.org/) ? "/" : "https://pbskids.org/" ;
  var path = "producerplayer/swf/" ;


  /******************************************
      org.pbskids.producerplayer PACKAGE
  ******************************************/
  if(!this.pbs) this.pbs = {};
  if(!pbs.kidsgo) pbs.kidsgo = {};
  if(!pbs.kidsgo.producerplayer) pbs.kidsgo.producerplayer = {};


  /******************************************
          producerplayer.init METHOD
  ******************************************/
  pbs.kidsgo.producerplayer = {
    init : function (_flashvars, _elementID) {
      var _that = pbs.kidsgo.producerplayer;
      
      /* BUILD THE PLAYER
      ******************************************/
        //-- CREATE CONTAINERS ----
        var isIE = navigator.appName.indexOf("Microsoft") != -1;
        var containerElement = (_elementID === undefined || _elementID === "undefined") ? undefined : (isIE) ? window[_elementID] : document.getElementById(_elementID);
        
        if(!containerElement){
          document.writeln('<div id="playerWrapper"></div>');
          containerElement = (isIE) ? window["playerWrapper"] : document.getElementById("playerWrapper");
        }
        containerElement.innerHTML = '<div id="producerplayer_flashcontent"></div>'; //Div replaced with FlashObject
        
                
        //-- ADD FLASH VARS, PARAMETERS, AND ATTRIBUTES ----
        var flashvars = _that.validateVars(_flashvars);
        var params = {};
            params.menu = "false";
        ...
        var container = _that.configContainer(flashvars);
        pbs.kidsgo.producerplayer.swfobject2_1.embedSWF(cdn + path + "ProducerPlayer.swf", "producerplayer_flashcontent", container.width, container.height, "10.1.0", "https://pbskids.org/pbsk/swf/expressInstall.swf", flashvars, params, attributes);
        
    },

    validateVars : function(flashvars){
      ...
      flashvars.releaseListWidth   = !isNaN(flashvars.releaseListWidth) ? Math.max(flashvars.releaseListWidth, releaseListMinWidth)
                                   : flashvars.dockReleaseList.toLowerCase() == "bottom" ? flashvars.playerWidth : releaseListDefaultWidth;
    ...
    configContainer : function(flashvars){
      var container = {};
