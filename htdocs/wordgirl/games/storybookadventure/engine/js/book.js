...
PBS.KIDS.storybook.book = function (GLOBAL, PBS, storybookContainerElement, config) {
	...
	var sb = PBS.KIDS.storybook,
        ...
        progressFill = $progressFill.get(0),
		...
		hideBrowserUi = function () {
			if (GLOBAL.isMobile.android.device) {
				GLOBAL.document.body.style.height = (GLOBAL.innerHeight + 76) + "px";
			} else if (GLOBAL.isMobile.apple.device) {
				GLOBAL.document.body.style.height = (GLOBAL.innerHeight + 56) + "px";
			} else {
		...
		updatePosition = function () {
			...
			if (curPageIndex === -1) {
				...
				document.body.className = document.body.className.replace(" on-internal", "");
				document.body.className = document.body.className.replace(" on-cover", "");
				document.body.className += ' on-cover';
			} else {
				...
				document.body.className = document.body.className.replace(" on-cover", "");
				document.body.className = document.body.className.replace(" on-internal", "");
				document.body.className += ' on-internal';
			}
		},
		...
		onNavigateComplete = function () {
			...
			if (targetPageIndex === -1) {
				...
				if (cover.pageSound) {
                    var snd = cover.pageSound;
                    var firstAudioPlay = setTimeout(function() {
                        audioPlayer.play(snd);
	                    window.clearTimeout(firstAudioPlay);
                    }, 1000);
				}
			} else {
            ...
            var targetPageIndexPlus = targetPageIndex + 1;
			...
			if (targetPageIndex === -1) {
				...
				if (curOrientation === "SINGLE-PAGE") {
                    ...
                    if(
                        (nextPageButtonElement) &&
                        (targetPageIndexPlus === 2 && !choice1made) ||
                        (targetPageIndexPlus === 4 && !choice2made) ||
                        (targetPageIndexPlus === 6 && !choice3made)) {
                    ...
                    if(
                        (nextPageButtonElement) &&
                        (targetPageIndexPlus === 1 && !choice1made) ||
                        (targetPageIndexPlus === 3 && !choice2made) ||
                        (targetPageIndexPlus === 5 && !choice3made)) {
                        nextPageButtonElement.style.display = "none";
                    }
				}
			}
		...
		start = function () {
            ...
            if (!GLOBAL.isMobile.apple.device && !GLOBAL.isMobile.android.device){
                GLOBAL.addEventListener("resize", updateLayout); //Though this was breaking iOS 5.1.1, it is still needed for desktop. -mm
            }
			...
			// Swaps loading hook with book-loaded class. Then dispatches an event.
		    document.body.className = 'book-loaded';
		    that.dispatchEvent("BOOK-LOADED");
		...
		updateLoadMessage = function (e) {
            ...
            var percentLoaded = Math.floor(100 * (loadEvent.progress/loadEvent.total)) + '%';
            $progressFill.css('width', percentLoaded);

			if (audioPlayer) {
				if (audioLoadInitiated) {
					if (audioLoadedEnough) {
                        $progressText.html(percentLoaded); // first item loading
					} else {
                        $progressText.html(percentLoaded); // normal loading
					}
				} else {
                    $progressText.html(percentLoaded); // press to load sound
				}
			} else {
                $progressText.html(percentLoaded);  // no audio loader
			}

            if(loadEvent.progress === loadEvent.total) {
                // all files loaded
            }
            else if (loadEvent.progress === loadEvent.total - 1) {
                // audio file needs to be loaded
                $('#progressWrapper').children().fadeOut();
                $('#readyToStart').fadeIn();
            }
		},
		...
		init = function () {
			...
			if (!initialized) {
				...
				if (config.audio && config.audio.name && config.audio.enabled !== false) {
					audioPlayer = sb.audioPlayer(GLOBAL, PBS, config.audio.path + config.audio.name);
                    PBS.KIDS.storybook.audio.player = audioPlayer;
				}
...
// TODO: Check if window.resize gets called on orientation change. If it is the following is unneccessary.
	    ...
	    // Adds ability to talk to sprites and triggers events
    that.getSprite = function (pageNumber, spriteIndex) {
        	// return sprite
        	return pages[pageNumber].getSprite(spriteIndex);
    };
