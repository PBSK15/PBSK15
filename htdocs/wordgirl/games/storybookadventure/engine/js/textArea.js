...
PBS.KIDS.storybook.textArea = function (GLOBAL, PBS, options) {
		...
		init = function () {
			...
			if (!initialized) {
				...
				for (i = 0; i < textArr.length; i += 1) {
                    ...
                    paragraphContent[i] = paragraphContent[i].replace(/\[u\](.*?)\[\/u\]/gi, "<u>$1</u>");
	...
	element = (textArr.length === 1) ? GLOBAL.document.createElement("div") : GLOBAL.document.createElement("div");
