		...
		if (config && config.content) {
			for (i = 0; i < config.content.length; i += 1) {
				...
				case "TextArea":
                    ...
                    if(config.content[i].showIf) {
                        PBS.KIDS.storybook.choices.bindElement({
                            selector : '.' + config.content[i].className,
                            showIf : config.content[i].showIf
                        });
                    }

                    if(config.content[i].addClass) {
                        config.content[i].className += ' ' + config.content[i].addClass;
                    }

					curTextArea = sb.textArea(GLOBAL, PBS, config.content[i]);

                    if(config.content[i].onPress) {
                        curTextArea.onPress = config.content[i].onPress;
                    }
				...
				case "Sprite":
                    ...
                    if(config.content[i].onPress) {
                        curSprite.onPress = config.content[i].onPress;
                    }
