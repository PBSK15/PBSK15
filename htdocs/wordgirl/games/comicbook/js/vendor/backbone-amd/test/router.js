$(document).ready(function() {
  ...
  test("routes (simple)", 4, function() {
    location.replace('https://example.com#search/news');
  ...
  test("routes (simple, but unicode)", 4, function() {
    location.replace('https://example.com#search/тест');
    Backbone.history.checkUrl();
  ...
  test("routes (two part)", 2, function() {
    location.replace('https://example.com#search/nyc/p10');
    Backbone.history.checkUrl();
  ...
  test("loadUrl is not called for identical routes.", 0, function() {
    ...
    location.replace('https://example.com#route');
  ...
  test("routes via navigate with {replace: true}", 1, function() {
    location.replace('https://example.com#start_here');
    Backbone.history.checkUrl();
    location.replace = function(href) {
      strictEqual(href, new Location('https://example.com#end_here').href);
    };
    Backbone.history.navigate('end_here', {replace: true});
  });

  test("routes (splats)", 1, function() {
    location.replace('https://example.com#splat/long-list/of/splatted_99args/end');
    Backbone.history.checkUrl();
    equal(router.args, 'long-list/of/splatted_99args');
  });

  test("routes (github)", 3, function() {
    location.replace('https://example.com#backbone/compare/1.0...braddunbar:with/slash');
    Backbone.history.checkUrl();
  ...
  test("routes (optional)", 2, function() {
    location.replace('https://example.com#optional');
    Backbone.history.checkUrl();
    ok(!router.arg);
    location.replace('https://example.com#optional/thing');
    Backbone.history.checkUrl();
    equal(router.arg, 'thing');
  });

  test("routes (complex)", 3, function() {
    location.replace('https://example.com#one/two/three/complex-part/four/five/six/seven');
    Backbone.history.checkUrl();
  ...
  test("routes (query)", 5, function() {
    location.replace('https://example.com#mandel?a=b&c=d');
    Backbone.history.checkUrl();
  ...
  test("routes (anything)", 1, function() {
    location.replace('https://example.com#doesnt-match-a-route');
    Backbone.history.checkUrl();
  ...
  test("routes (function)", 3, function() {
    ...
    location.replace('https://example.com#function/set');
    Backbone.history.checkUrl();
    equal(ExternalObject.value, 'set');
  });

  test("Decode named parameters, not splats.", 2, function() {
    location.replace('https://example.com#decode/a%2Fb/c%2Fd/e');
    Backbone.history.checkUrl();
  ...
  test("fires event when router doesn't have callback on it", 1, function() {
    ...
    location.replace('https://example.com#noCallback');
    Backbone.history.checkUrl();
  });

  test("#933, #908 - leading slash", 2, function() {
    location.replace('https://example.com/root/foo');
  ...
  test("correctly handles URLs with % (#868)", 3, function() {
    location.replace('https://example.com#search/fat%3A1.5%25');
    Backbone.history.checkUrl();
    location.replace('https://example.com#search/fat');
    Backbone.history.checkUrl();
  ...
  test("#1185 - Use pathname when hashChange is not wanted.", 1, function() {
    Backbone.history.stop();
    location.replace('https://example.com/path/name#hash');
    Backbone.history = _.extend(new Backbone.History, {location: location});
  ...
  test("#1206 - Strip leading slash before location.assign.", 1, function() {
    Backbone.history.stop();
    location.replace('https://example.com/root/');
    Backbone.history = _.extend(new Backbone.History, {location: location});
  ...
  test("Normalize root.", 1, function() {
    Backbone.history.stop();
    location.replace('https://example.com/root#fragment');
    Backbone.history = _.extend(new Backbone.History, {
      location: location,
      history: {
        pushState: function(state, title, url) {},
  ...
  test("Transition from hashChange to pushState.", 1, function() {
    Backbone.history.stop();
    location.replace('https://example.com/root#x/y');
    Backbone.history = _.extend(new Backbone.History, {
      location: location,
      history: {
        pushState: function(){},
  ...
  test("Transition from pushState to hashChange.", 1, function() {
    Backbone.history.stop();
    location.replace('https://example.com/root/x/y?a=b');
  ...
  test("#1695 - hashChange to pushState with search.", 1, function() {
    Backbone.history.stop();
    location.replace('https://example.com/root?a=b#x/y');
    Backbone.history = _.extend(new Backbone.History, {
      location: location,
      history: {
        pushState: function(){},
  ...
  test("#1980 - Optional parameters.", 2, function() {
    location.replace('https://example.com#named/optional/y');
    Backbone.history.checkUrl();
    strictEqual(router.z, undefined);
    location.replace('https://example.com#named/optional/y123');
    Backbone.history.checkUrl();
  ...
  test("#2062 - Trigger 'route' event on router instance.", 2, function() {
    ...
    location.replace('https://example.com#route-event/x');
    Backbone.history.checkUrl();
  });
