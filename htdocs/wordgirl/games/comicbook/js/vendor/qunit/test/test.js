...
function makeurl() {
	...
	var url = testEnv.url || 'https://example.com/search';
...
test("makeurl working", 3, function( assert ) {
	...
	assert.equal( makeurl(), 'https://example.com/search?q=a%20search%20test', 'makeurl returns a default url if nothing specified in the testEnvironment');
});
...
test("makeurl working with settings from testEnvironment", function( assert ) {
	assert.equal( makeurl(), 'https://google.com/?q=another_search_test', 'rather than passing arguments, we use test metadata to from the url');
});
