...
(function( require, define, factory ){
  ...
  function init(){
    ...
    $("#headband-container").prependTo("body").addClass("show-me"); // Added this to make sure the headband is injected in the correct location--no matter the placement of the globalinc
    
    var _set_body_class = function(){
      //Sets a unique class on the body -- for css override targeting
  ...
  *****************************/
  ...
  function onLocalized(e){
    if( e.station ){
      ...
      $("#headband-container .headband-nav .nav-item.station img").attr("src", e.station.smallbw || "");
    ...
    if( parentsbar.controller ){
      if( type == "pbskids.messages.site"){
        ...
        if( !!$(".parents-message.video").get(0) ){
          ...
          //For producer versions, only have 1 message so 
          //delete any messages before adding the video message
          clearMessages();
  ...
  *****************************/
  ...
  function expandHeadband(openOnly){
    ...
    if( $("#squished-headband-toggle").css("display") != "none" ) {
      if( headband_container.hasClass('expanded') ){
        if( !openOnly ){
          headband_nav.animate({
            "margin-top": -(headband_nav_height)
          }, 400, function() {
            headband_container.removeClass('expanded');
            $("body").removeClass("headband-open");
            headband_nav.css("margin-top","");//remove inline styles
  ...
  function updateVideoMessage( message ){
    ...
    if( message && message.series ) {
      ...
      $(".parents-message.video h3" ).html( "<span class='titles'>" + v.title + "</span> <i>(" + ( formatTime(v.duration) || "--:--" ) + ")</i> " );
        ...
        try{
           ...
           if (!__item.hasClass('video')) {
            ...
            if ($(window).width() <= 550){
              ...
              //message.width(message_total_text_width);
            } else {
              message.width(message_total_text_width * 0.8);
            }
  ...
  // Checks for GSM Virtual World Session Cookie &&  
  function checkExternalLogin(){ 
    var _href = window.location.href,
        _GSMregexp = /(\/gamestarmechanicjr)/g,
        _GSMCookieRegEx = /^(pbs_gsmjr_gsm_session=)/g;

    // CHECK GSM Login
    if ( _GSMregexp.test(_href) ){
      if ( hasCookieFromRegEx( _GSMCookieRegEx ) ){
        $("body").addClass('hide-pbskids-login');
        onLoggedOut();
      }
    }
  }
  
  function hasCookieFromRegEx( param ){
    var didGetCookie = false,
        cookies = document.cookie.split(';');
    
    if (param.constructor == RegExp){
      for ( var i = 0; i < cookies.length; i++ ){ 
        if ( param.test( cookies[i].trim() ) ){
          didGetCookie = true; 
        }
      };
    }
  ...
  //Callback Script Loader from Headband 1.0
  var loadDeprecatedCallback = function(){
