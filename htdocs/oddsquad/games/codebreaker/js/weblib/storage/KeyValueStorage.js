...
var KeyValueStorage = KeyValueStorage || new function () {
	...
	*/
	...
	function _cacheUpdateComplete(result) {
		//DEBUG
		//console.log("Received Key " + result.key + " with value " + result.value);		
		...
		if(_remainingKeys.length < 1) {
			//DEBUG
			//console.log("All keys loading - dispatching update complete");
	...
	_this.set = function(key, value, updateDB) {
		...
		if(typeof(Storage)!=="undefined" && _persistentStorageEnabled ) {
			try{
				...
				console.warn("Error while trying to set item in local storage: " + err.message);
			}
		} else {
		...
		if(updateDB && _persistentStorageEnabled) {
			...
			try {
				...
				console.warn("error getting value from db: " + e);
			}
		}
	};
	...
	**/
	_this.get = function (key, defaultValue) {
		...
		//console.log ( "checking local instead " + localStorage.getItem(key) );		
