...
//Declare the main Odd Squad namespace object if not already declared
if(ss.oddsquad === undefined){	
	ss.oddsquad = new Object();
}
	...
	*/
	function _UserLogin() {
		...
		*/
		...
		/*
		* Get the pbs user id
		* @return:[int] - The pbs user id if exist, otherwise null.
		*/
		_this.getPbsId = function() {
			return agent.id;
		}
		...
		*/
		_this.hasPbsLoginAndAgent = function(){
			//console.log("UserLoging: HasLogin: " + _this.hasPbsLogin() + ", HasAgent: " + _this.hasAgent());
		...
		*/
		function _handleLoginInfo(isPbsLoggedIn){
			//TESTING
			//console.log("UserLogin: Handle login info");

			//Set flag for whether user is logged in
			_hasPbsLogin = Boolean(org.pbskids.login.loggedin);
		...
		*/
		function _handleAgentDataLoaded(){
			...
			if(isEmpty(agent.name) || agent.name.length <= 0){
				...
				_hasAgent = true;
				_agentData = {id:agent.id, name:agent.name, code:agent.code, avatar:agent.avatar, background:agent.background};
			}
