...
(lib.PlumPuppet = function() {
	...
	// adding tween reference for plum float around
	this.float_tween;
...
//					console.log("child- " + this.instance.getChildIndex(floater));
	...
	this.floataround=function() {
		if(!this.nofloat) {
			...
			this.float_tween = cjs.Tween.get(this).to({x:newx,y:newy},2000,cjs.Ease.cubicInOut).call(this.floataround);
			this.float_tween.paused = false;
		}
		else {
			this.float_tween = null;
		}
	};
	
	// used to pause the float around for terrarium game in kart kingdom
	this.pauseFloatAround = function() {
		if(this.float_tween !== null && !this.float_tween.paused) {
			this.nofloat = true;
			this.float_tween.paused = true;
			this.float_tween.setPaused(true);
		}
	};
	
	// used to restart the float around for terrarium game in kart kingdom
	this.restartFloatAround = function() {
		if(this.float_tween !== null && this.float_tween.paused) {
			this.nofloat = false;
			this.float_tween.paused = false;
			this.float_tween.setPaused(false);
		}
	};
